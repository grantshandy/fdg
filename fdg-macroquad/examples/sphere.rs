...
async fn main() {
    ...
    for y in 0..height {
        for x in 0..width {
            if x != 0 {
                graph.add_edge(indices[(width * y) + x], indices[((width * y) + x) - 1], JsonValue::default());
            }

            if y != 0 {
                graph.add_edge(indices[(width * y) + x], indices[(width * (y - 1)) + x], JsonValue::default());
            }
        }

        // cylinder
        graph.add_edge(indices[(width * y) + (width - 1)], indices[(width * y)], JsonValue::default());
    }

    for x in 0..width {
        graph.add_edge(indices[(width * (height - 1)) + x], top, JsonValue::default());
